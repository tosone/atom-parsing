...
class Parsing extends Snapshot {
  ...
  globalFun() {
    this.evt.on('interaction/internal/collect/story-status-changed', data => { // 引擎内部的信息收集和外部的信息抛出
      ...
      this.sdk.event.emit('interaction/collect', {
  ...
  checkSelected(flag) {
    if (flag[0] === '{') {
      ...
      try {
        ...
        let flagList = _.isString(parseFlag)
          ? [parseFlag.complexFlagList]
          : parseFlag.complexFlagList;
      ...
      return this.selected.indexOf(flag) === -1
        ? true
        : false;
    }
  }
  ...
  init(setting) {
    ...
    this.manifestFile = setting.manifest
      ? setting.manifest
      : this.config.manifest; // 关于故事执行逻辑的相关描述文件
    return new Promise((resolve, reject) => {
      ...
      if (fs.existsSync(path.join(this.filepath, this.packageFile))) {
        ...
        reject({
          cmd: 'story crashed',
          info: 'Can not find out file in ' + path.join(this.filepath, this.manifestFile) + '.'
        });
      }
      ...
      if (!setting.manifest && !this.networkstatus && this.storyPackageInfo.offlineManifest)
        this.manifestFile = this.storyPackageInfo.offlineManifest;
