...
class Test {
  constructor() {
    ...
    this.interaction_dir = path.join(__dirname, 'example', 'test');
  ...
  init() {
    this.mqtt.on('connect', () => {
      this.log('Parsing test connect MQTT succ.');
    ...
    this.repl.defineCommand('mode', {
      ...
      action: dir => {
        let directory = dir !== '' ? path.join(__dirname, 'example', dir) : this.interaction_dir;
    ...
    this.repl.defineCommand('start', {
      ...
      action: dir => {
        let directory = dir !== '' ? dir : this.interaction_dir;
        let payload = JSON.stringify({ dir: directory });
        this.mqtt.publish(this.interaction_start, payload);
        this.repl.displayPrompt();
      }
