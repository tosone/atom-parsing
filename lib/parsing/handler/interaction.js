...
class Interaction {
  constructor(result) {
    ...
    this.nfc_evt = null;
    this.backline = null;
    this.interaction_bgm = this.config.interaction_bgm; // 在互动活动的过程中的一些背景音
  }

  init() {
    return new Promise(resolve => {
      this.evt.once('interaction_end', data => {
        let ret = this.playList;
        if (this.backline) {
          ret = { lists: this.playList, backline: this.backline };
        }
        this.mqtt.publish('sound_manager/pause', JSON.stringify({}), () => { // 停止在等待用户按键或者nfc过程中的背景音
          if (data !== 'null') {
            resolve(ret); // 正常结束，返回当前的
          } else {
            this.clear(); // 异常的结束，清理一些对象按键nfc监听
          }
        });
      });
      let interactionOnce = true; // 保证有互动的流程
      _.forEach(this.lists, action => { // 遍历所用的互动输入模式查找相应的参数
        if (action.interactionBgm) {
          this.interaction_bgm = action.interactionBgm;
        }
      });
      if (this.lists.length !== 0) {
        player(fileResolve(this.result, this.interaction_bgm), 0, 'break');
      }
      _.forEach(this.lists, action => { // 遍历所有的互动类型
        if (action.cmd === 'button') { // 按键的互动流程
          ...
          this.button_timer = setTimeout(() => { // 按键的超时处理
            ...
            if (activity) { // 当存在没有按键触发的逻辑，作为默认的流程
            ...
            if (keyType === 'keydown') {
              _.forEach(keysbind, k => {
                if (k === key && activies[key]) {
                  ...
                  this.evt.emit('interaction/internal/collect/story-command', { command: key });
                  let activity = activies[key];
                  if (activity.LED) {
                    LED(ledAlias(activity.LED.light), activity.LED.mode, activity.LED.option);
                  }
                  if (activity.backline) {
                    this.backline = Number(activity.backline);
                  }
                  if (activity.sound) {
          ...
          this.nfc_evt = code => {
            if (_.indexOf(this.nfcBindings, code) !== -1) {
              _.forEach(this.nfcBindings, v => { // 遍历所有的nfc卡的绑定
                if (v === code) {
                  this.evt.emit('interaction/internal/collect/story-command', { command: v });
