...
class SingleComplex {
  ...
  init() {
    this.evt.on('timer/sound', () => { // 关于音频的定时触发
      this.onSound();
    });
    this.evt.on('timer/LED', () => { // 关于 LED 的定时触发
      this.onLED();
    });
    if (this.activies) {
    ...
    this.evt.on('button', (key, keyType) => {
    ...
    return new Promise(resolve => {
      ...
      this.evt.once('singleComplex_end', () => {
        this.reset();
        resolve(this.playList);
      });
    });
  ...
  onLED(args) {
    if (args.mode) {
      LED(ledAlias(args.light), args.mode, args.options);
    }
  }
  ...
  reset() {
    ...
    this.evt.removeListener('timer/sound', this.onSound);
    this.evt.removeListener('timer/LED', this.onLED);
    this.evt.removeListener('button', this.keyEvt);
  }
}
