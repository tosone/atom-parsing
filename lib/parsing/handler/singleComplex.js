...
class SingleComplex {
  constructor(result) {
    ...
    if (this.sentence.last) {
      this.last = timeStr(this.sentence.last);
    }
    this.backline = null;
    if (this.sentence.backline) {
      this.backline = Number(this.sentence.backline);
    }
  ...
  init() {
    this.timer_sound_evt = data => { this.onSound(data); };
    this.evt.on('timer/sound', this.timer_sound_evt); // 关于音频的定时触发

    this.timer_LED_evt = data => { this.onLED(data); };
    this.evt.on('timer/LED', this.timer_LED_evt); // 关于 LED 的定时触发

    if (this.keysbinding.length !== 0) {
      this.button_evt = (key, keyType) => { this.keyEvt(key, keyType); };
      this.evt.on('button', this.button_evt); // 关于按键的定时触发
    }
  ...
  reset() {
    ...
    if (this.timer_sound_evt) this.evt.removeListener('timer/sound', this.timer_sound_evt);
    if (this.timer_LED_evt) this.evt.removeListener('timer/LED', this.timer_LED_evt);
