...
class Library {
  constructor() {
    ...
    this.nfc_topic = 'interaction/command';

    this.network_topic = 'network/bbcloudonline/status';
  ...
  btn(buttonKey) {
    ...
    this.mqtt.publish(this.button_topic, payload);
  }

  btnhold(buttonKey) {
    let keyCode = (buttonKey !== '' ? convertButton.R1A.keyCode[buttonKey] : 'key12') || 'key12';
    let payload = JSON.stringify({ keyCode, para: 'long' });
    this.mqtt.publish('input/keyboard/keyhold', payload);
  }

  nfc(code) {
    let nfcId = code !== '' ? code : '字母卡a';
    let payload = JSON.stringify({ type: 'nfc', data: { nfcAlias: nfcId } });
    this.mqtt.publish(this.nfc_topic, payload);
  }

  networkstatusChange(status) {
    let networkstatus = status !== '' ? status : null;
    if (networkstatus) networkstatus = networkstatus === 'off' ? null : networkstatus;
    this.mqtt.publish(this.network_topic, JSON.stringify({ satus: networkstatus ? true : false }));
  }
}
