...
export default {
  ...
  panel(title) {
    let panel = new MessagePanelView({
      rawTitle: true,
      title: '<span style="color:#a6e22e;font-size:16px;">Interaction Autotest generate Status for \'' + title + '\'</span>',
      maxHeight: '500px',
      autoScroll: true
    });
    panel.attach();
  ...
  toggle() {
    ...
    if (editor && title && title === 'manifest.rc') {
      let pushMsg = (str) => {
        panel.add(new PlainMessageView({message: str, raw: true}));
      }
      evt.on('line', data => {
      ...
      if (!panel.panel.visible)
        panel.attach();
      panel.clear();
      mqtt.publish('interaction/start', JSON.stringify({
